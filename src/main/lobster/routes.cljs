...
(defn- user-handler [{:keys [params]}]
  (-> (lobster/user (:username params))
      (.then (fn [user]
               (let [invited-by-username (:invited_by_user user)
                     links (cond-> (hal/user-links "lobsters" (:username params))
                             invited-by-username
                             (assoc :invited_by (hal/link (str "/lobsters/users/" invited-by-username))))]
                 (hal/resource
                  links
